<nav class="navbar navbar-light bg-light" style="margin-bottom:20px;background-color:#cfe1f2 !important;">
	<a class="navbar-brand"><img src="../css/bootstrap-solid.svg" height="30" class="d-inline-block align-top"> SRMS</a>
	<div class="my-sm-0">
		<a href="" id="back"><button type="button" class="btn btn-secondary btn-sm">Back</button></a>
		<a href="../logout.php"><button type="button" class="btn btn-danger btn-sm">Logout</button></a>
	</div>		
</nav>